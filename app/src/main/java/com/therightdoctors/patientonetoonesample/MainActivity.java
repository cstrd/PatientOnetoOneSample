...
import com.therightdoctors.patientonetoonesample.ui.PreviewCameraFragment;
import com.therightdoctors.patientonetoonesample.ui.PreviewControlFragment;
import com.therightdoctors.patientonetoonesample.ui.RemoteControlFragment;
...
public class MainActivity extends AppCompatActivity implements PreviewControlFragment.PreviewControlCallbacks,
        RemoteControlFragment.RemoteControlCallbacks, PreviewCameraFragment.PreviewCameraCallbacks/*, SignalListener*/ {
   ...
   public String s="asdf",t="sdfsef",bookingId,tkbxapi="45810982",type;
    //Permissions
   ...
   public  String token="ssadfa",session="asdfa",tokboxkey="45810982";
  //  DatabaseReference rootref,childref;
    SharedPreferences sharedpreferences;
    String did;
    OTConfig config;
    int i=0;
    String SIGNAL_TYPE="Started";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
      ...
      /*  sharedpreferences = getSharedPreferences("mypreference",
        ...
        childref=rootref.child("doctors").child(did).child("live_video");
        childref.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                String ft=dataSnapshot.child("token").getValue(String.class);
                String fs=dataSnapshot.child("session").getValue(String.class);
                token=ft;
                session=fs;
        ...
        s=getIntent().getStringExtra("session");
        bookingId=getIntent().getStringExtra("booking_id");
        tkbxapi=getIntent().getStringExtra("tkbxapi");
        type=getIntent().getStringExtra("type");
        //remote and local audio only view
     /*   System.out.println(s+t);
        Toast.makeText(this,s+t, Toast.LENGTH_SHORT).show();*/
        //remote and local audio only view
        ...
        //init the wrapper
        OTConfig config =
                new OTConfig.OTConfigBuilder(s, t,tkbxapi).name("one-to-one-sample-app").subscribeAutomatically(false).subscribeToSelf(false).build();
        if (config != null) {
    ...
    private BasicListener mBasicListener =
            new PausableBasicListener(new BasicListener<OTWrapper>() {
                ...
                public void onPreviewViewReady(OTWrapper otWrapper, View localView) throws ListenerException {
                    ...
                    if(type.equals("tele")){ setLocalView(null);}else{  setLocalView(localView); }
                }
                @Override
                ...
                public void onRemoteViewReady(OTWrapper otWrapper, View remoteView, String remoteId, String data) throws ListenerException {
                    ...
                    if ( remoteId == mRemoteId ) {
                        if (isCallInProgress()) {
                            if(type.equals("tele")){ setRemoteView(null);}else{  setRemoteView(remoteView); }
                        }
